...
+};
+
static void __init rb711gr100_init_partitions(const struct
rb_info *info)
{
rb711gr100_spi_partitions[0].size = info->hard_cfg_offs;
rb711gr100_spi_partitions[1].offset = info-
>hard_cfg_offs;
-
- rb711gr100_spi_partitions[3].offset = info-
>soft_cfg_offs;
+ rb711gr100_spi_partitions[2].offset = info-
>soft_cfg_offs;
}
...
MIPS_MACHINE_NONAME(ATH79_MACH_RB_711GR100,
...
+
+ info = rb_init_info((void *) KSEG1ADDR(0x1f000000),
0x20000);
+ if (!info)
+ return;
+
...
+ rb711gr100_spi_flash_data.nr_parts =
ARRAY_SIZE(rb711gr100_spi_partitions);
+ }
+ ath79_register_spi(&rb711gr100_spi_data,
rb711gr100_spi_info, 1);
+
+
...
+
+ if (!nand_disabled) {
+ printk(KERN_NOTICE "using NAND flash for
root\n");
+ platform_device_register_data(NULL, "rb91xnand",
-1,
...
ARRAY_SIZE(rbgroove_leds_gpio),
+ rbgroove_leds_gpio);
+}
+
+MIPS_MACHINE(ATH79_MACH_RB_GROOVE, "groove-52",
"MikroTik 52HPn",
+ rbgroove_setup);
